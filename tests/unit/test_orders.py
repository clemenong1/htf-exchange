...
class TestOrderInitialisation:
    def test_limit_order_creation(self):
        ...
        order = LimitOrder(order_id=1, side="buy", price=100, qty=10, user_id=None)
    ...
    def test_market_order_creation(self):
        ...
        order = MarketOrder(order_id=2, side="sell", qty=5, user_id=None)
    ...
    def test_invalid_order_quantity_zero(self):
        ...
        with pytest.raises(ValueError, match="Order quantity must be > 0"):
            LimitOrder(1, "buy", 100, 0, user_id=None)
        with pytest.raises(ValueError, match="Order quantity must be > 0"):
            MarketOrder(1, "buy", 0, user_id=None)

    def test_invalid_order_quantity_negative(self):
        """Test that negative quantity raises ValueError."""
        with pytest.raises(ValueError, match="Order quantity must be > 0"):
            LimitOrder(1, "buy", 100, -5, user_id=None)
        with pytest.raises(ValueError, match="Order quantity must be > 0"):
            MarketOrder(1, "buy", -5, user_id=None)
